...
from utils.util import \
    accuracy, adjust_learning_rate, save_checkpoint, load_model, AverageMeter,\
    IMAGE_MEAN_VALUE, IMAGE_STD_VALUE, calculate_IOU, draw_bbox, save_images, \
    show_layers
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer):
    ...
    for i, (images, target) in enumerate(train_t):
        ...
        if i < 5 and args.gpu == 0:
            writer.add_image(args.name,
                             show_layers(images, model, target, args),
                             epoch * 5 + i)
