...
def show_layers(images, model, target, args):
    """ Return an image which is composed of results intermediate layers.
    The image has grid shape of images where the each row means a layer.
    This first row is always input image.
    """
    grid_image = None
    ...
    image_ = images.clone().detach() * image_std_value + image_mean_value
    ...
    if args.distributed:
        print_layers = model.module.get_layers()
    else:
        print_layers = model.get_layers()
    for idx, (name, layer) in enumerate(print_layers.items()):
        normalize, scale_each = True, True
        if name == 'attention':
            normalize, scale_each = False, False
        if 'feat_map' in name:
            layer = layer[range(b), target].unsqueeze(1)

        if layer.size(1) != 1:
            layer = torch.mean(layer, dim=1, keepdim=True)
        layer = F.interpolate(layer, (h // 2, w // 2),
                              mode='bilinear', align_corners=False)
        layer = vutils.make_grid(layer[:16], nrow=16,
                                 normalize=normalize, scale_each=scale_each)
        if idx == 0:
            grid_image = torch.cat((image_set, layer), dim=1)
        else:
            grid_image = torch.cat((grid_image, layer), dim=1)
    return grid_image